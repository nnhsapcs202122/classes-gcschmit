...
public class MileageTrackerTest
{
    ...
    public void testIncrementMilesDriven()
    {
         ...
         *   For values of type double (only for doubles), specify a third
         *      argument which is the epsilon (i.e., how close is close
         *      enough to be equal).
         */
        assertEquals(10.0, amount, 1e-6 /* 1 x 10^-6 */);
        
        // best practice: invoke method multiple times for a more robust test
        testTracker.incrementMilesDriven(5.0);
        amount = testTracker.getMilesDriven();
        assertEquals(15.0, amount, 1e-6);
    }
    ...
    {
        MileageTracker testTracker = new MileageTracker();
        testTracker.incrementMilesDriven(100.0);
        testTracker.incrementFuelConsumed(4.0);
